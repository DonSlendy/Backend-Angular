...
public class UserRepository implements IUserRepository {
    ...
    public UserModel getByNameAndEmail(UserModel userModel){
        List<UserModel> user = jdbcTemplate.query("SELECT * FROM user WHERE firstname = ? OR email = ?",
                new Object[]{userModel.getFirstname(),userModel.getEmail()},new BeanPropertyRowMapper<>(UserModel.class));
        if(user.isEmpty()){
            return null;
        }
        return user.get(0);
